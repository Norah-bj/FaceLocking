...
def list_people(cfg: EvalConfig) -> List[Path]:
    ...
    return sorted(
        [p for p in cfg.enroll_dir.iterdir() if p.is_dir()]
    )
...
def load_embeddings_for_person(
    ...
    for img_path in imgs:
        ...
        # If someone accidentally saved non-aligned crops, skip them
        # (keeps eval clean)
        if (
            cfg.require_size is not None
            and not _is_aligned_crop(img, cfg.require_size)
        ):
            continue
    ...
    if same:
        for i in range(len(embs_a)):
            for j in range(i + 1, len(embs_a)):
                dists.append(
                    cosine_distance(embs_a[i], embs_a[j])
                )
    else:
        for ea in embs_a:
            for eb in embs_b:
                dists.append(
                    cosine_distance(ea, eb)
                )
...
def main():
    ...
    print(
        f"Genuine (same person): {describe(genuine)}"
    )

    print(
        f"Impostor (diff persons): {describe(impostor)}"
    )
